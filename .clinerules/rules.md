...
This project uses: fastapi, openai, perplexity, websocket, uvicorn, gitpython, chrome, mcp

## ARCHITECTURE

- **Project type**: fastapi-api
- **Entry points**: src\main.py
- **Structural patterns**: fastapi-api, pytest-tests
- **Languages**: python
...
**Python** (5 packages): fastapi, uvicorn, openai, gitpython, websockets
...
- Store API keys in `.env` or environment variables — never hardcode (openai, perplexity)
...
- Don't use sync functions for I/O in async route handlers
...
1. Chat interface in extension (text + future voice support)
2. Browser automation: Navigate GitHub, GitHub Actions, scrape logs/diffs
3. Backend analysis: Perplexity API + git diff + MCP for local tools
